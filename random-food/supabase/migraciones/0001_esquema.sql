...
returns trigger
language plpgsql
-- `set search_path` incluso aquí: el Security Advisor de Supabase marca toda
-- función que no lo fije, y una función de disparador sin acotar es un vector
-- de suplantación de esquema.
...
-- Código de invitación con entropía real.
--
-- `random()` NO es criptográfico: su semilla es por backend, y Supabase reutiliza
-- backends del pool, así que la secuencia es predecible. `gen_random_uuid()` usa
-- pg_strong_random y es del núcleo desde PG13, sin extensiones.
--
-- 12 caracteres hexadecimales son 48 bits. Con 6 el espacio era de 16 millones:
-- alguien que se registra (el registro es abierto) podía barrerlo entero contra
-- el RPC de unirse y colarse en CUALQUIER hogar, no en uno concreto.
...
comment on column public.hogares.codigo_invitacion is
  ...
  'y se puede regenerar con rotar_codigo_invitacion() si se filtra.';
...
create table public.platos (
  ...
  -- son válidos.
  --
  -- NO ACTION y no RESTRICT: ambos impiden borrar una categoría en uso, pero
  -- RESTRICT se comprueba fila a fila, así que al borrar un hogar (que borra en
  -- cascada categorías Y platos) saltaba según el orden en que Postgres
  -- procesara las filas. NO ACTION difiere la comprobación al final de la
  -- sentencia, cuando los platos ya han desaparecido: el borrado de un hogar
  -- funciona y borrar una categoría suelta en uso sigue fallando.
...
-- Solo entre los platos activos: si no, archivar «Lentejas» dejaba el nombre
-- quemado para siempre y no se podía volver a crear.
...
create index platos_creado_por_idx on public.platos (creado_por);
...
comment on column public.platos.archivado is
  'Los platos se archivan en vez de borrarse: borrarlos de verdad se lleva por '
  'delante sus apariciones en el calendario (plan_comidas cae en cascada), y con '
  'ellas el historial. La aplicación solo archiva; el borrado definitivo queda '
  'para el panel de Supabase, a sabiendas.';
...
-- Este índice sirve también para filtrar por hogar_id, que es su primera
-- columna: no hace falta uno aparte.
...
create table public.plato_ingredientes (
  ...
  -- numeric(12,3): con dos decimales, 0.005 kg se redondeaba a 0.01 y una
  -- cantidad menor que 0.005 se redondeaba a 0, violando su propio CHECK > 0.
...
create trigger plato_ingredientes_actualizado_en
  before update on public.plato_ingredientes
  for each row execute function public.tocar_actualizado_en();
...
create table public.plan_comidas (
  ...
  -- huecos vacíos. El `nullif(trim(...))` es necesario: con `notas <> null` a
  -- secas, una cadena vacía pasaba el check y dejaba el hueco ocupado por una
  -- fila que la interfaz pintaba en blanco y que el generador no podía rellenar.
  constraint plan_hueco_no_vacio
    check (plato_id is not null or nullif(trim(notas), '') is not null),
...
create index plan_creado_por_idx   on public.plan_comidas (creado_por);
